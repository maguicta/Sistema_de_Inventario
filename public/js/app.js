...
async function fetchSettings() {
  try {
    ...
    const fullStoreName = branch ? `${name} (${branch})` : name;
    ...
    if (settings.store_logo) {
       document.querySelector('.sidebar-logo i').style.display = 'none';
       document.querySelector('.sidebar-logo').innerHTML = `<img src="${settings.store_logo}" style="width:40px;height:40px;border-radius:8px;object-fit:cover;margin-right:12px;" id="sidebar-logo-img"> <span class="sidebar-title" id="sidebar-store-name">${fullStoreName}</span>`;
    }
      ...
      <div class="pos-products">
        <div class="pos-products-header">
          <div class="sidebar-logo">
            ...
            <span id="sidebar-store-name">${settings.store_name || 'Inventario Pro'}</span>
          </div>
...
async function loadUsers(container) {
  ...
  container.innerHTML = `
    <div class="table-container">
      ...
      <table>
        <thead><tr><th>Usuario</th><th>Email</th><th>Rol</th><th>Permisos</th><th>Estado</th><th>Acciones</th></tr></thead>
        <tbody>
          ${users.map(u => `
            <tr>
              <td>
...
async function backupDB() {
  ...
  try {
    // En una app real esto dispararía una ruta de descarga
    // Por ahora simularemos la exportación de un JSON con todo
    const data = {
    ...
    a.href = url;
    a.download = `backup_${settings.store_name || 'tienda'}_${new Date().toISOString().slice(0,10)}.json`;
    a.click();
...
async function loadSettings(container) {
  ...
  container.innerHTML = `
    <div class="settings-container" style="max-width: 800px; margin: 0 auto; padding: 20px;">
      <div class="dashboard-card">
        <div class="dashboard-card-header">
          <h3><i class="fas fa-store"></i> General</h3>
        </div>
        <div class="dashboard-card-body">
          <form id="settings-form" onsubmit="saveSettings(event)">
            <div class="form-row">
              ...
              <div class="form-group">
                ...
                <input type="text" id="set-branch_name" value="${settings.branch_name || ''}" placeholder="Ej: Santa Fe">
              </div>
            </div>
            <div class="form-row">
              <div class="form-group">
                ...
                <select id="set-theme" class="filter-select" style="width:100%;padding:10px 16px;">
                  <option value="midnight" ${settings.theme === 'midnight' ? 'selected' : ''}>Medianoche (Deep Dark)</option>
                  <option value="aura" ${settings.theme === 'aura' ? 'selected' : ''}>Aura (Púrpura)</option>
                  <option value="forest" ${settings.theme === 'forest' ? 'selected' : ''}>Bosque (Esmeralda)</option>
                  <option value="ocean" ${settings.theme === 'ocean' ? 'selected' : ''}>Océano (Azul)</option>
                  <option value="light" ${settings.theme === 'light' ? 'selected' : ''}>Polar (Clarito)</option>
                </select>
              </div>
              <div class="form-group">
                <label>Logo de la Tienda (URL)</label>
                <div style="display:flex;gap:10px;">
                   <input type="text" id="set-store_logo" value="${settings.store_logo || ''}" placeholder="https://...">
            ...
            <div class="form-row">
              <div class="form-group">
                <label>Moneda Base (Sistema)</label>
                <select id="set-currency_base" class="filter-select" style="width:100%;padding:12px 16px;">
                  ...
                  <optgroup label="Latinoamérica">
                    <option value="ARS" ${settings.currency_base === 'ARS' ? 'selected' : ''}>Peso Argentino (ARS)</option>
                    <option value="BOB" ${settings.currency_base === 'BOB' ? 'selected' : ''}>Boliviano (BOB)</option>
                    <option value="BRL" ${settings.currency_base === 'BRL' ? 'selected' : ''}>Real Brasileño (BRL)</option>
                    <option value="CLP" ${settings.currency_base === 'CLP' ? 'selected' : ''}>Peso Chileno (CLP)</option>
                    <option value="COP" ${settings.currency_base === 'COP' ? 'selected' : ''}>Peso Colombiano (COP)</option>
                    <option value="CRC" ${settings.currency_base === 'CRC' ? 'selected' : ''}>Colón Costarricense (CRC)</option>
                    <option value="CUP" ${settings.currency_base === 'CUP' ? 'selected' : ''}>Peso Cubano (CUP)</option>
                    <option value="DOP" ${settings.currency_base === 'DOP' ? 'selected' : ''}>Peso Dominicano (DOP)</option>
                    <option value="GTQ" ${settings.currency_base === 'GTQ' ? 'selected' : ''}>Quetzal Guatemalteco (GTQ)</option>
                    <option value="HNL" ${settings.currency_base === 'HNL' ? 'selected' : ''}>Lempira Hondureña (HNL)</option>
                    <option value="MXN" ${settings.currency_base === 'MXN' ? 'selected' : ''}>Peso Mexicano (MXN)</option>
                    <option value="NIO" ${settings.currency_base === 'NIO' ? 'selected' : ''}>Córdoba Nicaragüense (NIO)</option>
                    <option value="PAB" ${settings.currency_base === 'PAB' ? 'selected' : ''}>Balboa Panameño (PAB)</option>
                    <option value="PYG" ${settings.currency_base === 'PYG' ? 'selected' : ''}>Guaraní Paraguayo (PYG)</option>
                    <option value="PEN" ${settings.currency_base === 'PEN' ? 'selected' : ''}>Sol Peruano (PEN)</option>
                    <option value="UYU" ${settings.currency_base === 'UYU' ? 'selected' : ''}>Peso Uruguayo (UYU)</option>
                    <option value="VES" ${settings.currency_base === 'VES' ? 'selected' : ''}>Bolívar Venezolano (VES)</option>
                  </optgroup>
                </select>
              </div>
              <div class="form-group">
            ...
            <div class="form-row">
              <div class="form-group">
                <label>Comparativa / Alternativa (Tasa)</label>
                <select id="set-currency_secondary" class="filter-select" style="width:100%;padding:12px 16px;">
            ...
            <div class="form-row">
              <div class="form-group">
                <label>Tasa de Cambio (1 Ref = X Base)</label>
                <div style="display:flex;gap:8px;">
                   <input type="text" id="set-exchange_rate" value="${settings.exchange_rate || '1.0000'}">
                   <button type="button" class="btn btn-ghost" onclick="updateAutoExchangeRate()" style="padding:0 12px;"><i class="fas fa-sync-alt"></i></button>
                </div>
              </div>
            <div class="form-row">
              <div class="form-group">
              ...
              <div class="form-group">
                <label>Auto-actualizar Tasa (minutos)</label>
                <input type="number" id="set-exchange_refresh_interval" value="${settings.exchange_refresh_interval || '0'}" placeholder="0 = desactivado">
                <small style="color:var(--text-muted);font-size:0.7rem;">0 para desactivar</small>
              </div>
            </div>
            <div class="form-group">
              <label style="display:flex;align-items:center;gap:10px;cursor:pointer;">
                <input type="checkbox" id="set-allow_negative_stock" ${settings.allow_negative_stock === 'true' ? 'checked' : ''} style="width:auto;">
                <span>Permitir facturación sin stock (inventario negativo)</span>
              </label>
            </div>
            <div class="modal-actions">
...
async function saveSettings(e) {
  ...
  const data = {
    branch_name: document.getElementById('set-branch_name').value,
  ...
  container.innerHTML = `
    ...
    <div class="dashboard-card" style="margin-top:20px;">
       ...
       <div class="dashboard-card-body" style="display:flex;gap:20px;flex-wrap:wrap;">
          ...
          <button class="btn btn-ghost" onclick="toast('Generando PDF...', 'info')"><i class="fas fa-file-download"></i> Inventario de Productos (PDF)</button>
          <button class="btn btn-ghost" onclick="backupDB()"><i class="fas fa-database"></i> Descargar Backup de Base de Datos</button>
          <button class="btn btn-ghost" onclick="toast('Generando Reporte...', 'info')"><i class="fas fa-chart-line"></i> Reporte de Ventas Mensual</button>
       </div>
    </div>
  `;
}
