...
router.post('/', canEdit, async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO products (name, description, sku, barcode, category_id, supplier_id, price, cost, stock, min_stock, unit, image_url, expiration_date) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [name, description || '', sku, barcode || null, category_id || null, supplier_id || null, price, cost || 0, stock || 0, min_stock || 5, unit || 'unidad', image_url || null, expiration_date || null]
    );
