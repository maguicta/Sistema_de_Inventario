...
router.get('/stats', async (req, res) => {
  try {
    ...
    const [lowStock] = await pool.query("SELECT COUNT(*) as count FROM products WHERE stock <= min_stock AND status = 'active'");
    const [todaySales] = await pool.query(
      "SELECT COUNT(*) as count, COALESCE(SUM(total), 0) as total FROM sales WHERE DATE(created_at) = CURDATE() AND status = 'completada'"
    );
    const [monthSales] = await pool.query(
      "SELECT COUNT(*) as count, COALESCE(SUM(total), 0) as total FROM sales WHERE MONTH(created_at) = MONTH(CURDATE()) AND YEAR(created_at) = YEAR(CURDATE()) AND status = 'completada'"
    );
    const [inventoryValue] = await pool.query(
      "SELECT COALESCE(SUM(price * stock), 0) as retail_value, COALESCE(SUM(cost * stock), 0) as cost_value FROM products WHERE status = 'active'"
    );
    const [monthExpenses] = await pool.query(
      "SELECT COALESCE(SUM(amount), 0) as total FROM expenses WHERE MONTH(created_at) = MONTH(CURDATE()) AND YEAR(created_at) = YEAR(CURDATE())"
    );
...
router.get('/sales-chart', async (req, res) => {
  try {
    const [data] = await pool.query(`
      ...
      FROM sales WHERE created_at >= DATE_SUB(CURDATE(), INTERVAL 30 DAY) AND status = 'completada'
...
router.get('/top-products', async (req, res) => {
  try {
    const [data] = await pool.query(`
      ...
      WHERE s.created_at >= DATE_SUB(CURDATE(), INTERVAL 30 DAY) AND s.status = 'completada'
