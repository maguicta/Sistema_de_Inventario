...
router.get('/', async (req, res) => {
  try {
    ...
    let query = `SELECT s.*, u.name as user_name FROM sales s LEFT JOIN users u ON s.user_id = u.id WHERE s.branch_id = ?`;
...
router.get('/:id', async (req, res) => {
  try {
    const [sales] = await pool.query(
      `SELECT s.*, u.name as user_name FROM sales s LEFT JOIN users u ON s.user_id = u.id WHERE s.id = ? AND s.branch_id = ?`,
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const [countResult] = await conn.query(
      `SELECT COUNT(*) as count FROM sales WHERE DATE(created_at) = CURDATE() AND branch_id = ?`, [req.user.branch_id || 1]
    );
    ...
    const [result] = await conn.query(
      `INSERT INTO sales (invoice_number, user_id, customer_name, subtotal, total, payment_method, amount_paid, change_amount, status, notes, branch_id) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'completada', ?, ?)`,
      [invoiceNum, req.user.id, customer_name || 'Cliente General', subtotal, total, payment_method || 'efectivo', amount_paid || total, changeAmount, notes || null, req.user.branch_id || 1]
    );
