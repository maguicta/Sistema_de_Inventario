...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const [result] = await conn.query(
      `INSERT INTO purchase_orders (order_number, user_id, supplier_id, subtotal, total, status, notes) 
       VALUES (?, ?, ?, ?, ?, 'pendiente', ?)`,
      [orderNum, req.user.id, supplier_id, subtotal, total, notes || null]
    );
...
router.patch('/:id/status', async (req, res) => {
  ...
  try {
    ...
    const [orders] = await conn.query('SELECT * FROM purchase_orders WHERE id = ?', [req.params.id]);
