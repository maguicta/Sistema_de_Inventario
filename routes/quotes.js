...
router.get('/', async (req, res) => {
  try {
    ...
    let query = `SELECT q.*, u.name as user_name FROM quotes q LEFT JOIN users u ON q.user_id = u.id WHERE q.branch_id = ?`;
...
router.get('/:id', async (req, res) => {
  try {
    const [quotes] = await pool.query(
      `SELECT q.*, u.name as user_name FROM quotes q LEFT JOIN users u ON q.user_id = u.id WHERE q.id = ? AND q.branch_id = ?`,
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const [countResult] = await conn.query(
      `SELECT COUNT(*) as count FROM quotes WHERE DATE(created_at) = CURDATE() AND branch_id = ?`, [req.user.branch_id || 1]
    );
    const quoteNum = `COT-${dateStr}-${String(countResult[0].count + 1).padStart(3, '0')}`;

    const [result] = await conn.query(
      `INSERT INTO quotes (quote_number, user_id, customer_name, subtotal, total, status, expiration_date, notes, branch_id) 
       VALUES (?, ?, ?, ?, ?, 'pendiente', ?, ?, ?)`,
      [quoteNum, req.user.id, customer_name || 'Cliente General', subtotal, total, expiration_date || null, notes || null, req.user.branch_id || 1]
    );
...
router.patch('/:id/status', async (req, res) => {
  try {
    ...
    await pool.query('UPDATE quotes SET status = ? WHERE id = ? AND branch_id = ?', [status, req.params.id, req.user.branch_id || 1]);
